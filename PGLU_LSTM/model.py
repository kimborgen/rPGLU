...
class PGLU_LSTM(nn.Module):
    def __init__(self, params):
        super(PGLU_LSTM, self).__init__()

        # PGLU FCNN
        self.W_i = nn.Linear(params.input_size, params.hidden_size)
        self.pglu = PGLU(params)

        self.lstm = nn.LSTMCell(input_size=params.hidden_size, hidden_size=params.hidden_size, bias=True)

    def forward(self, x, h_prev, c_prev,potential_prev): 
        # Linear layer for input gate
        processed_input = self.W_i(x)
        activated, potential = self.pglu(processed_input, potential_prev)

        h_new, c_new = self.lstm(activated, (h_prev, c_prev))           
        return h_new, c_new, potential
    
class PGLU_LSTM_net(nn.Module):
    def __init__(self, params, device):
        super(PGLU_LSTM_net, self).__init__()
    ...
    def forward(self, data):
        ...
        for step in range(data.size(0)):  # data.size(0) = number of time steps
            ...
            hidden1, c1, pot1 = self.pglu1(x, hidden1, c1, pot1)
            #d1 = self.d1(hidden1)
...
@dataclass
class PGLU_LSTM_Params(ExperimentParams):
    hidden_size: int
...
def get_pglu_lstm_params():
    input_size = 10

    return PGLU_LSTM_Params(
        model_id="pglu_lstm",
        short_name="pglu_lstm_last",
...
def train_gated():
    params = get_pglu_lstm_params() 
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    print("Using device: ", device)

    experiements = [
        {"init_decay_center": 0.5, "init_decay_sigma": 0.1, "init_tresh_center": 0, "init_tresh_sigma": 0, "hidden_size": 128},
    ]
    ...
    for exp in experiements:
        ...
        net = PGLU_LSTM_net(params, device).to(device)
